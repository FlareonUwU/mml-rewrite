--[[
Hi! This is LuaModelMaker's Admin V3 by MakerModelLua. Rewrote by iiCookiesF0rLife
...
local Workspace,workspace,Instance,require,script = game.Workspace,workspace,Instance,require,script

wait(1)

local ExploitModule = game:FindFirstChild("MakerMode".."lLua's Admin Anti-Exploit Module", true)

if Server.Players.LocalPlayer then print("MML Admin Rewrite Compromised: Running in studio") IsStudio = true end
...
function REMOTE:TrelloGetAdminBoard()
	...
	if Boards then
		for _,Board in pairs(Boards) do
			if Board.name == "MakerModelLua's Admin Rewrite V3" and Board.closed == false then
				ReturnBoard = Board
				break
			end
		end
...
function MARKET:SearchItem(Category, Query, Subcategory, CatalogContext)
	local URLAdd = ""
	...
	--Use Roblox's site directly, classy-studios.com no longer has the CatalogSearch endpoint
	local JSON = REMOTE:GetURL("https://search.roblox.com/catalog/json?CatalogContext="..Server.HS:UrlEncode(CatalogContext).."&Category="..Server.HS:UrlEncode(Category).."&Keyword="..Server.HS:UrlEncode(Query).."&ResultsPerPage=25") --Set a cap at 25 so it doesnt overwork
	if JSON then
...
>RUNCODE:
return {
	local Hack = {"Account", "Items", "Robux"};
...
Commands = { -- can't make local
	...
	[{"forceplace"}] = {"player num(UserID)", "Forces a player to teleport to the place num(UserID) is at(Only works in universes at this time)", 2, "Owner", false, function(Speaker, Rank, Arguments)
		local Players = STRING:Scan(Arguments[1], Speaker)
	...
	[{"place"}] = {"player num(UserID)", "Prompts a player to teleport to the place num(UserID) is at(Only works in universes at this time)", 2, "Owner", false, function(Speaker, Rank, Arguments)
		local Players = STRING:Scan(Arguments[1], Speaker)
	...
	[{"nuke"}] = {"player", "Nukes the player", 1, "Admin", false, function(Speaker, Rank, Arguments)
		local Players = STRING:Scan(Arguments[1], Speaker)
		for _,Player in pairs(Players) do
			CORE:Nuke(Player)
		end
	...
	[{"nogod", "ungod", "nopower", "unpower"}] = {"player", "Resets health to full, undoes god effect(Doesn't mean there isn't God)", 1, "Member", false, function(Speaker, Rank, Arguments)
		local Players = STRING:Scan(Arguments[1], Speaker)
		for _,Player in pairs(Players) do
			if Player.Character and Player.Character:FindFirstChild("Humanoid") then
	...
	[{"loopheal"}] = {"player", "Loops player's health to full (May cause lag)", 1, "Member", false, function(Speaker, Rank, Arguments)
		local Players = STRING:Scan(Arguments[1], Speaker)
		for _,Player in pairs(Players) do
			if Player.Character and Player.Character:FindFirstChild("Humanoid") then
				local Char = Player.Character
...
KeyPressed.OnServerInvoke = function(Player, Key)
	if Player and Key then
		if KeyBinds[Player.UserId] and KeyBinds[Player.UserId][Key] then
			CORE:Chatted(KeyBinds[Player.UserId][Key], Player, true)
		end
	end
end

InputKey.OnServerInvoke = function(Player, KeyVal)
	if Player and KeyVal then
		if LastKeys[Player.UserId] then
			table.insert(LastKeys[Player.UserId], KeyVal)
			if #LastKeys[Player.UserId] > 20 then
				table.remove(LastKeys[Player.UserId], 1)
			end
		end
	end
end
